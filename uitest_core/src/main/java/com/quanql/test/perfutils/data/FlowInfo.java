...
public class FlowInfo {
	...
	public void getCurrentFlowData() {
		if (!appPackageName.isEmpty()) {
			...
			this.printLine = StringUtils
					.join(new String[] { String.valueOf(DoubleUtil.div(this.getlReceive_Wlan0_Bytes(), 1024 * 1024, 4)),
							String.valueOf(DoubleUtil.div(this.getlTransmit_Wlan0_Bytes(), 1024 * 1024, 4)),
							String.valueOf(
									DoubleUtil.div(this.getlReceive_Wlan0_Bytes() + this.getlTransmit_Wlan0_Bytes(),
											1024 * 1024, 4)) },
							",");
		}
	}
	...
	public void getCurrentFlowData(String appPkgName) {
		...
		for (String str : cr.getSuccessMsgArray()) {
			if (str.contains("wlan0")) {
				...
				String join = StringUtils.join("接收/发送(bytes):", lReceive_Wlan0_Bytes, " / ", lTransmit_Wlan0_Bytes);
				LogUtil.debug(join);
			}
		}
	}
	...
	public void countFlow(String firstFlow, String secondFlow) {
		...
		this.printLine = StringUtils.join(new String[] { String.valueOf(new BigDecimal(receiveFlow).setScale(4, 4)),
