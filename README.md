...
两种用例编写方式
---
我将参与UI自动化用例编写的人分为两类，（当然，也可能有的团队不会分的这么细，一个人齐活）
1. 第一类：编写底层/基础功能的人员，他们可能负责编写base中公共方法、识别和封装元素，这类人可以不了解业务，但对编程技能要求高；
2. 第二类：业务用例编写人员，将页面、组件按照一定的业务要求进行整合，使之符合一定的story，完成自动化的业务测试，而他们使用的页面和元素就是第一类人员提供的，这类人要求熟悉业务，对编程技能要求不高。

工程中提供两了种用例编写的方式，第一种对用例编写人员的更友好、技术要求不高、可读性强。我个人更习惯第二种，对复杂控件和场景支持更好。
 ...
 * 2. 再对元素进行操作，比如，LoginPage.UserName.sendkeys("111111") 输入内容，sendkeys()是公共方法，并不是在page封装时提供的。但是这里需要注意！用例编写人员也可以对这个编辑框进行.click()，而另一种编写方式是不允许这样做的。
 * 这是和另一个方式的第二处区别。
 */
...
/**
 * 说明：
 * 注意这里不仅需要识别页面元素，还需要完成元素事件的封装，比如点击按钮。
 */
