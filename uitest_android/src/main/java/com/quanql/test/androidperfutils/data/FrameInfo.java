...
public class FrameInfo { //FIXME 改成针对帧集合，而不是单个帧
	...
	private static String printTitle = "DrawAvg,PrepareAvg,ProcessAvg,ExecuteAvg,TotalAvg,TotalFrameCnt,DropFrameCnt,DropFrameRate";
	...
	public void getCurrentFrameData() {
		...
		CommandResult cr = ShellUtils.execCommand("adb shell dumpsys gfxinfo " + Constant.APP_PACKAGE_NAME);
...
//				} else 
		...
		if (0 == iFrameTotal) {
			...
			this.printLine = dDrawAVG / iFrameTotal + "," + dPrepareAVG / iFrameTotal + "," + dProcessAVG / iFrameTotal + ","
					+ dExecuteAVG / iFrameTotal + "," + dTotalAVG / iFrameTotal + "," + iFrameTotal + "," + junkCount + ","
