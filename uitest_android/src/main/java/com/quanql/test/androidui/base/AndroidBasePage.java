...
public class AndroidBasePage extends BasePage {
	...
	public static String getCurrentActivity() {
		return ((AndroidDriver<MobileElement>) baseOpt.getDriver()).currentActivity();
	}
	 ...
	 * 
	 * @param key
	 *            即com.quanql.demo.R.id.key中的key
	...
	public static void scrollToUp(int loop, int period) {
		...
		for (int i = 0; i < loop; i++) {
			...
			baseOpt.swipe(Constant.SCREEN_WIDTH / 2, Constant.SCREEN_HEIGHT / 5 * 4, Constant.SCREEN_WIDTH / 2,
					Constant.SCREEN_HEIGHT / 5, Constant.SCROLL_MEDIUM);
			baseOpt.wait(period);
		}
	}
	...
	public static void scrollToDown(int loop, int period) {
		...
		for (int i = 0; i < loop; i++) {
			baseOpt.swipe(Constant.SCREEN_WIDTH / 2, Constant.SCREEN_HEIGHT / 4, Constant.SCREEN_WIDTH / 2,
					Constant.SCREEN_HEIGHT / 4 * 3, Constant.SCROLL_MEDIUM);
			baseOpt.wait(period);
		}
	}
	...
	public static void scrollToLeft(int loop, int period) {
		...
		for (int i = 0; i < loop; i++) {
			baseOpt.swipe(Constant.SCREEN_WIDTH / 6 * 5, Constant.SCREEN_HEIGHT / 2, Constant.SCREEN_WIDTH / 6,
					Constant.SCREEN_HEIGHT / 2, Constant.SCROLL_MEDIUM);
			baseOpt.wait(period);
		}
	}
	...
	public static void scrollToRight(int loop, int period) {
		...
		for (int i = 0; i < loop; i++) {
			baseOpt.swipe(Constant.SCREEN_WIDTH / 6, Constant.SCREEN_HEIGHT / 2, Constant.SCREEN_WIDTH / 6 * 5,
					Constant.SCREEN_HEIGHT / 2, Constant.SCROLL_MEDIUM);
			baseOpt.wait(period);
		}
	}
